...
av_sd_y_diff %>%
  # group_by(thisLat, thisLong, thisCrop, thisCultivar, 
  #          Adaptation, Altitude_masl) %>%
 # summarise(DiffValue = mean(mean)) %>%
  dplyr::select(-sd) %>%
  tidyr::spread(thisCrop, mean) %>%
  ggplot(aes(x=maize, y=wheat, colour=factor(thisCultivar))) +
  geom_point(aes(size = as.numeric(Altitude_masl), shape=thisCultivar), alpha=0.8) +
  ...
  geom_errorbar(aes(ymin=len-ci, ymax=len+ci), colour="grey", width=.1) +
