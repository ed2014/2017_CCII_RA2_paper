...
df_worked <- df_raw %>%
  dplyr::select(-X,-X.1, -skip) %>% # FIXME: Find why you have this in raw data (likely due to previous merge?)
  ...
  mutate(thisScenario = factor(thisScenario, levels=c("base", "midCentury", "endCentury"), 
...
# get elevation data
```{r LoadAdapt, echo = FALSE, include=TRUE}
# merge altitude data
...
summary(alt_info)
```
# Add total biomass and merge

```{r}

df_worked <- merge(df_worked, alt_info, by = c("thisLat", "thisLong"))
...
```{r}
df_worked <- merge(df_worked, lu_info, by = c("row", "col"))
...
FIXME: Why only maize here? - this is not used at the moment - check later if needed

```{r GetEraMaizeYields}

# TotBioMaize_ERA <- df_worked %>%
#   group_by(row, col,thisSoil, thisRCP, thisCrop, thisCultivar, thisCultivar, thisGCM) %>%
#   filter(thisRCP == "ERA") %>%
#   filter(thisCrop == "maize") %>% # why not do it for both?
#   dplyr::select(row, col,thisSoil, thisRCP, thisCrop, thisCultivar, thisCultivar, thisGCM, TotalBiomass) %>%
#   mutate(TotalBiomassERA = TotalBiomass) %>%
#   dplyr::select(-TotalBiomass) %>%
#   summarise_each(funs(mean)) %>%
#   dplyr::select(-thisRCP, -thisGCM)
# 
# summary(TotBioMaize_ERA)

```

# Checking silage yields at lowlands
```{r}
# TotBioMaize_ERA %>%
#   filter(row == 68 & col == 198)
```
