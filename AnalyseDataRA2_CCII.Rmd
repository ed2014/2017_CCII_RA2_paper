...
```
## Get extra explanatory variables for t
...
tb_base <- table_support_vars %>%
  ...
  dplyr::select( -Altitude_masl,-year,-AdaptLabel,-ScenLabel,-thisLat,-thisLong) %>%
  summarise_each(funs(min,mean,max)) %>%
...
spread(GroupVar, VarValue) %>%
  ...
  group_by(VarName, thisCrop) %>%  
  dplyr::select(-thisLat, -thisLong) %>%
  summarise_each(funs(min,mean,max)) %>% # FIXME: do quantiles
  ...
  ungroup() %>%
  mutate(VarName=factor(VarName, levels=orF)) # FIXME: how to print table in factor order instead of alphabetical?
